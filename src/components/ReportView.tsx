...
> = {
  ...
  lab: {
    label: "Lab / Exploración",
    icon: "🔬",
    color: "text-[#0d6e6e]",
    bg: "bg-[#f0fafa]",
    border: "border-[#d0ecec]",
  },
  studio: {
    label: "Studio",
    icon: "🛠",
...
> = {
  base: {
    label: "Estado base",
    description: "Pesos canónicos del escenario sin modificar",
  },
  adjusted: {
    label: "Estado ajustado",
    description: "Pesos modificados por el usuario en Lab",
  },
  comparison: {
    label: "Comparación base vs ajustado",
    description:
      "Análisis comparativo entre estado base y estado con pesos modificados",
  },
};
...
export function ReportView({
  ...
  const sDiscarded =
    data.narrative.discardedOptions.length > 0 ? nextSection() : 0;
  ...
  return (
    <div className="report-container max-w-[800px] mx-auto bg-white print:shadow-none print:max-w-none print:bg-white print:p-0">
      <PrintCoverPage
            ...
            {originInfo && (
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className={cn("text-sm font-bold", originInfo.color)}>
                    {originInfo.label}
                  </p>
                </div>
              </div>
      ...
      <header className="border-b-2 border-[#111827] pb-6 mb-8">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right flex-shrink-0">
            <p className="text-xs font-bold text-[#0d6e6e] tracking-wider uppercase">
              Convergia
            </p>
      ...
      <section className="mb-8">
        ...
        {data.winnerName ? (
          ...
          <p className="text-red-600 font-medium">
            No se pudo determinar una opción ganadora tras las rondas de
            negociación.
          </p>
        )}
      </section>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b-2 border-slate-900">
                <th className="text-left py-2 pr-4 font-bold text-slate-900">
                  Stakeholder
                </th>
                <th className="text-left py-2 pr-4 font-bold text-slate-900">
                  Rol
                ...
                <th className="text-center py-2 pr-4 font-bold text-slate-900">
                  Score
                </th>
                <th className="text-center py-2 font-bold text-slate-900">
                  Estado
                </th>
              </tr>
            </thead>
      ...
      <section className="mb-8">
        ...
        {data.concessions.length === 0 ? (
          <p className="text-slate-500 text-sm">
            No hubo concesiones — todos los stakeholders mantuvieron sus
                ...
                <tbody>
                  {data.concessions.map((c, i) => (
                    <tr key={i} className="border-b border-slate-200">
                      ...
                      <td className="py-2 pr-4 font-medium text-slate-900">
                        {data.stakeholderNames[c.stakeholderId] ??
                          c.stakeholderId}
                      </td>
                      <td className="py-2 pr-4 text-slate-700">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b-2 border-slate-900">
                  ...
                  <th className="text-left py-2 font-bold text-slate-900">
                    Motivo
                  </th>
                </tr>
              </thead>
        ...
        <section className="mb-8 break-before-page">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="border border-slate-200 rounded-lg p-4">
              ...
              <p className="text-lg font-bold text-slate-900">
                {data.comparison.baseWinnerName}
              </p>
              <p className="text-xs text-slate-500 font-mono">
                Consenso: {data.comparison.baseConsensusScore.toFixed(3)} ·
                Conflicto: {data.comparison.baseConflict.toFixed(3)}
              </p>
            </div>
            <div
              className={cn(
                "border rounded-lg p-4",
                data.comparison.winnerChanged
            ...
            >
              ...
              <p className="text-lg font-bold text-slate-900">
                {data.comparison.adjustedWinnerName}
              </p>
              <p className="text-xs text-slate-500 font-mono">
                Consenso: {data.comparison.adjustedConsensusScore.toFixed(3)} ·
                Conflicto: {data.comparison.adjustedConflict.toFixed(3)}
              </p>
            </div>
          </div>
          <div className="overflow-x-auto">
      ...
      <footer className="border-t-2 border-[#111827] pt-4 mt-12">
        <div className="flex items-center justify-between text-xs text-[#5b6578]">
          <div>
            ...
            {enrichment?.source === "ai" && (
              <p className="text-violet-500 mt-1">
                ✨ Narrativa enriquecida con IA (redacción solamente —
          ...
          <div className="text-right">
            ...
            <p>
              {data.roundCount} rondas · {data.stakeholderCount} stakeholders ·
