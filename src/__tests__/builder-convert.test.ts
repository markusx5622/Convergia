...
function makeStakeholder(
  overrides: Partial<DraftStakeholder> = {},
): DraftStakeholder {
  return {
    uid: "sh-1",
    name: "Producción",
...
function makeState(overrides: Partial<BuilderState> = {}): BuilderState {
  return {
    ...
    stakeholders: [
      makeStakeholder({ uid: "sh-1", name: "SH1" }),
      makeStakeholder({ uid: "sh-2", name: "SH2" }),
    ],
    options: [
      makeOption({ uid: "opt-1", name: "Opt A" }),
      makeOption({ uid: "opt-2", name: "Opt B", cost: "60000" }),
    ],
    ...overrides,
  };
}
...
describe("buildSingleStakeholder", () => {
  ...
  it("uses fallback defaults for invalid concession params", () => {
    ...
    expect(result.concessionThreshold).toBe(0.1);
    expect(result.concessionRate).toBe(0.12);
    expect(result.acceptabilityThreshold).toBe(0.3);
  });
});
