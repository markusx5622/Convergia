...
function validWeights(): Record<VariableId, string> {
  ...
  const sum = VARIABLE_IDS.slice(0, -1).reduce(
    (acc, v) => acc + parseFloat(w[v]!),
    0,
  );
...
function validState(): BuilderState {
  return {
    ...
    stakeholders: [
      validStakeholder("SH1", "Role1"),
      validStakeholder("SH2", "Role2"),
    ],
...
describe("validateStakeholder", () => {
  ...
  it("allows weights within tolerance", () => {
    ...
    const sum = VARIABLE_IDS.reduce(
      (acc, v) => acc + parseFloat(s.weights[v]),
      0,
    );
  ...
  it("flags invalid acceptability threshold", () => {
    ...
    const errs = validateStakeholder(s, 0);
    expect(errs.some((e) => e.path.includes("acceptabilityThreshold"))).toBe(
      true,
    );
  });
...
describe("validateOption", () => {
  ...
  it("flags cost exceeding budget", () => {
    ...
    const errs = validateOption(o, 0, 100000);
    expect(
      errs.some((e) => e.path.includes("cost") && e.message.includes("excede")),
    ).toBe(true);
  });
...
describe("validateAll", () => {
  ...
  it("flags fewer than 2 stakeholders", () => {
    ...
    expect(
      errs.some((e) => e.message.includes("al menos 2 stakeholders")),
    ).toBe(true);
  });
  ...
  it("flags fewer than 2 options", () => {
    ...
    const errs = validateAll(s);
    expect(errs.some((e) => e.message.includes("al menos 2 opciones"))).toBe(
      true,
    );
  });
  ...
  it("flags duplicate stakeholder names", () => {
    const s = validState();
    s.stakeholders = [
      validStakeholder("DuplicateName", "R1"),
      validStakeholder("DuplicateName", "R2"),
    ];
    const errs = validateAll(s);
  ...
  it("flags duplicate option names", () => {
    const s = validState();
    s.options = [
      validOption("Same Name", "50000"),
      validOption("Same Name", "60000"),
    ];
    const errs = validateAll(s);
    expect(errs.some((e) => e.message.includes("duplicados"))).toBe(true);
  });
