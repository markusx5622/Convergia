      ...
      customWeights
        ? canonicalStakeholders.some((s, i) =>
            VARIABLE_IDS.some(
              (v) =>
                Math.abs(
                  s.weights[v] - (customWeights[i]?.[v] ?? s.weights[v]),
                ) > 0.005,
            ),
          )
        : false,
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="mt-2 text-[#5b6578] text-base leading-relaxed max-w-2xl">
                Entorno libre de exploración: selecciona un escenario, ajusta
                los pesos de los stakeholders y observa cómo cambia la decisión.
                No hay orden fijo — explora a tu ritmo. Todo sigue siendo
                determinista.
              </p>
        ...
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center gap-4 flex-wrap">
          ...
          <span className="text-xs font-mono text-slate-400">
            ...
            <strong className="text-slate-700">
              {adjustedResult.finalOption?.name ?? "—"}
            </strong>
          </span>
        </div>
      </div>
          ...
          <div className="space-y-8">
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm card-interactive">
              ...
              <p className="text-slate-600 leading-relaxed mb-4">
                {bundle.scenario.description}
              </p>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {bundle.scenario.kpis.map((kpi) => (
                  <div
                    key={kpi.name}
                    className="bg-slate-50 rounded-lg border border-slate-200 p-3 text-center transition-all duration-200 hover:bg-[#f0fafa] hover:border-[#d0ecec]"
                  >
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 stagger-children">
                {bundle.options.map((opt) => (
                  <div
                    key={opt.id}
                    className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm card-interactive"
                  >
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-[#111827] mb-1">
                    Ajuste de pesos por stakeholder
                  </p>
                  <p className="text-xs text-[#5b6578] leading-relaxed">
                    Mueve los sliders para explorar cómo cambia el resultado al
                    dar más peso a un stakeholder u otro. Los pesos siempre
                    suman 100% — al mover uno, los demás se reajustan
                    proporcionalmente. El resultado se recalcula de forma
                    determinista en cada cambio.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-[#f0fafa] rounded-xl border border-[#d0ecec] p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-[#111827] mb-1">
                    Comparación base vs ajustado
                  </p>
                  <p className="text-xs text-[#5b6578] leading-relaxed">
                    El resultado &quot;base&quot; usa los pesos canónicos del
                    escenario. El resultado &quot;ajustado&quot; usa los pesos
                    que has configurado en la pestaña anterior. Ambos son
                    deterministas.
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm mt-6">
              ...
              <p className="text-xs text-slate-500 mb-4">
                Elige qué tipo de informe quieres generar a partir de tu
                exploración actual.
              </p>
      ...
      <footer className="border-t border-[#e1e4eb] bg-white mt-auto">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <p className="text-xs text-[#5b6578]">
            Convergia Lab · Sandbox analítico · Exploración no lineal · Motor
            determinista · 2025
          </p>
          <div className="flex items-center gap-3">
            <Link
